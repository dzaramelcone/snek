...
pub fn nextMessage(ring: *const TransportRing, logical_off: usize) ParseError!?MessageView {
    if (ring.remainingFrom(logical_off) < 5) return null;

    const header = if (ring.sliceIfContiguous(logical_off, 5)) |header_bytes|
        try wire.readMessageHeader(header_bytes)
    else blk: {
        var header_bytes: [5]u8 = undefined;
        ring.copyInto(logical_off, &header_bytes);
        break :blk try wire.readMessageHeader(&header_bytes);
    };
    const total_len = 1 + @as(usize, @intCast(header.length));
    if (total_len > ring.capacity()) return error.MessageTooLarge;
    if (ring.remainingFrom(logical_off) < total_len) return null;
...
pub fn applyRowDescription(
    ...
    if (ring.sliceIfContiguous(payload_off, payload_len)) |payload| {
    ...
    const field_count = readU16At(ring, payload_off);
    ...
    for (0..field_count) |i| {
        ...
        while (pos < payload_end and ring.byteAt(pos) != 0) : (pos += 1) {}
        ...
        const type_oid = readU32At(ring, pos + 6);
        if (!stmt_entry.described) {
            stmt_entry.col_keys[i] = try unicodeFromLogicalRange(allocator, ring, name_start, name_len);
...
pub fn materializeDataRow(
    ...
    const field_count = try parseDataRowSpans(ring, payload_off, payload_len, &spans);
    const render_count = @min(col_count, field_count);
    ...
    const copySpanField = struct {
        fn f(ctx: SpanCopyCtx, idx: usize, dest: []u8) void {
            ctx.ring.copyInto(ctx.spans[idx].logical_off, dest);
        }
    }.f;
    ...
    const row_obj = snek_row.createCopied(
        ...
        render_count,
        pool,
        .{ .ring = ring, .spans = spans[0..render_count] },
        spanFieldLen,
        copySpanField,
    ) catch null;
    ...
    for (0..render_count) |i| {
        ...
        const value_obj = try unicodeFromLogicalRange(allocator, ring, spans[i].logical_off, spans[i].len);
        defer ffi.decref(value_obj);
        try ffi.dictSetItem(dict, name, value_obj);
    }
...
pub fn parseCommandCompleteCount(ring: *const TransportRing, payload_off: usize, payload_len: usize) i64 {
    if (ring.sliceIfContiguous(payload_off, payload_len)) |payload| {
        return parseCommandCompleteTagCount(wire.parseCommandComplete(payload));
    }

    var end = payload_len;
    while (end > 0 and ring.byteAt(payload_off + end - 1) == 0) {
        end -= 1;
    }
    if (end == 0) return 0;

    var start = end;
    while (start > 0 and ring.byteAt(payload_off + start - 1) != ' ') {
        start -= 1;
    }
    ...
    for (start..end) |logical_idx| {
        const ch = ring.byteAt(payload_off + logical_idx);
...
fn parseDataRowSpans(
    ...
    for (0..field_count) |i| {
        ...
        const val_len = readI32At(ring, pos);
        pos += 4;
...
fn unicodeFromLogicalRange(
    ...
    if (ring.sliceIfContiguous(logical_off, len)) |slice| {
        return ffi.unicodeFromSlice(slice.ptr, slice.len);
    }

    const bytes = try allocator.alloc(u8, len);
    defer allocator.free(bytes);
    ring.copyInto(logical_off, bytes);
    return ffi.unicodeFromSlice(bytes.ptr, bytes.len);
}

fn readU16At(ring: *const TransportRing, logical_off: usize) u16 {
    var bytes: [2]u8 = undefined;
    ring.copyInto(logical_off, &bytes);
    return mem.bigToNative(u16, mem.bytesToValue(u16, &bytes));
}

fn readU32At(ring: *const TransportRing, logical_off: usize) u32 {
    var bytes: [4]u8 = undefined;
    ring.copyInto(logical_off, &bytes);
    return mem.bigToNative(u32, mem.bytesToValue(u32, &bytes));
}

fn readI32At(ring: *const TransportRing, logical_off: usize) i32 {
    return @bitCast(readU32At(ring, logical_off));
}
