...
pub fn GroupReadOutputType(comptime events: []const Event) type {
    var fields: [events.len]Type.StructField = undefined;
...
pub fn Group(comptime events: []const Event) type {
    ...
    return struct {
        ...
        pub fn read(self: *Self) Error!Output {
            ...
            if (std.posix.errno(rc) != .SUCCESS) return error.ReadGroupFailed;
        ...
        fn perf_open_group(group_fd: linux.fd_t, config: u64) Error!linux.fd_t {
            ...
            if (std.posix.errno(rc) != .SUCCESS) return error.OpenGroupFailed;
            return @intCast(rc);
        }
        ...
        fn ioctl_get_id(fd: linux.fd_t) Error!u64 {
            ...
            if (std.posix.errno(rc) != .SUCCESS) return error.GetIdFailed;
            return id;
        }
        ...
        fn ioctl_reset_group(fd: linux.fd_t) Error!void {
            ...
            if (std.posix.errno(rc) != .SUCCESS) return error.ResetGroupFailed;
        }
        ...
        fn ioctl_enable_group(fd: linux.fd_t) Error!void {
            ...
            if (std.posix.errno(rc) != .SUCCESS) return error.EnableGroupFailed;
        }
        ...
        fn ioctl_disable_group(fd: linux.fd_t) Error!void {
            ...
            if (std.posix.errno(rc) != .SUCCESS) return error.DisableGroupFailed;
        }
    };
}
