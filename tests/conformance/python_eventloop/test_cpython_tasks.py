"""Upstream CPython task/future coverage adapted for snek.

Adapted from CPython `Lib/test/test_asyncio/test_tasks.py`.
Only the harness and implementation matrix are changed; test bodies are kept
as close to upstream as possible while targeting `snek.loop`.
"""
...
def _load_snek_native():
    try:
        from snek import _snek
    except Exception as exc:  # pragma: no cover - environment specific
        raise AssertionError(
            "snek native task/future types are not available. Build or install "
            "the extension before running the CPython task conformance suite."
        ) from exc
    return _snek
...
class SnekTaskTestCase(test_utils.TestCase):
    @staticmethod
    def close_loop(loop):
        loop.close()
    ...
    def setUp(self):
        super().setUp()
        asyncio.set_event_loop_policy(_load_snek_loop().EventLoopPolicy())
    ...
    def new_test_loop(self, gen=None):
        if gen is not None:
            self.skipTest(
                "snek.loop does not support CPython TestLoop time generators yet"
            )
        loop = _load_snek_loop().new_event_loop()
...
class GenericTaskTests(SnekTaskTestCase):

    def test_future_subclass(self):
        native = _load_snek_native()
        self.assertIsSubclass(native.Task, native.Future)

    def test_native_module_importable(self):
        native = _load_snek_native()
        self.assertHasAttr(native, "Task")
        self.assertHasAttr(native, "Future")
...
class FutureGatherTests(GatherTestsBase, SnekTaskTestCase):
...
class CoroutineGatherTests(GatherTestsBase, SnekTaskTestCase):
...
class RunCoroutineThreadsafeTests(SnekTaskTestCase):
...
class SleepTests(SnekTaskTestCase):
    def setUp(self):
        super().setUp()
        self.loop = asyncio.new_event_loop()
        self.set_event_loop(self.loop)
...
class CompatibilityTests(SnekTaskTestCase):
...
class SnekTaskFutureTests(BaseTaskTests, SetMethodsTest, SnekTaskTestCase):
    Task = _load_snek_native().Task
    Future = _load_snek_native().Future
    all_tasks = staticmethod(asyncio.all_tasks)
    current_task = staticmethod(asyncio.current_task)


class SnekIntrospectionTests(SnekTaskTestCase, BaseTaskIntrospectionTests):
    _register_task = staticmethod(getattr(tasks, "_register_task", tasks._py_register_task))
    _unregister_task = staticmethod(getattr(tasks, "_unregister_task", tasks._py_unregister_task))
    _enter_task = staticmethod(getattr(tasks, "_enter_task", tasks._py_enter_task))
    _leave_task = staticmethod(getattr(tasks, "_leave_task", tasks._py_leave_task))
    all_tasks = staticmethod(asyncio.all_tasks)
    current_task = staticmethod(asyncio.current_task)
    Task = _load_snek_native().Task


class SnekCurrentLoopTests(BaseCurrentLoopTests, SnekTaskTestCase):
    current_task = staticmethod(asyncio.current_task)
