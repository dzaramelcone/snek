...
def _emit_single_method(query: Query, tables: dict[str, Table]) -> list[str]:
    ...
    if query.params:
        ...
        if include_model:
            lines.append(f"        return _snek.{op}({const_name}, {tuple_args}, {model_name})")
        else:
            lines.append(f"        return _snek.{op}({const_name}, {tuple_args})")
    else:
        if include_model:
            lines.append(f"        return _snek.{op}({const_name}, (), {model_name})")
        else:
            lines.append(f"        return _snek.{op}({const_name}, ())")
...
def _emit_batch_method(query: Query, tables: dict[str, Table]) -> list[str]:
    ...
    if include_model:
        lines.append(f"        return _snek.{op}({const_name}, ({param_args},), {model_name})")
    else:
        lines.append(f"        return _snek.{op}({const_name}, ({param_args},))")
    lines.append("")
    return lines
